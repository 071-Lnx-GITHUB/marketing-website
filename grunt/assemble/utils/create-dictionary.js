...
module.exports = function(assemble) {
  ...
  /**
   * Array will only be translated if key defining the array has a TR flag
   ...
   * for translation. If it contains an object only those keys with TR will be processed
   *
   * @param {Array} `arr` Array to be processed
   * @param {String} `type` Type of translation key (TR)
  ...
  /**
   * Utility function to determine if the key/value pair represents entire (HTML) flagged for translation
   ...
   * @return {Object|Boolean} returns the page content `val` or `false`
   */
  function translatePageContent(key, split, fileObj) {
    var suffix = split[1];
    var pageContent, val;
  ...
  var createDictionary = function(fileData, locale) {
    ...
    // TODO: point links to the right place
    ...
    return Object.keys(data).reduce(function(o, key) {
      ...
      var val, recursed;
      var isPageContent = translatePageContent(key, split, fileData);
