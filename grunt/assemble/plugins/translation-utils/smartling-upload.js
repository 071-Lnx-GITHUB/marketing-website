...
module.exports = function(assemble) {
  ...
  // TODO: config files don't exist (commented out in _assemble.js)
  // var smartlingConfigFile = options.smartlingConfigs[env];
  // console.log('CURRENT SMARTLING CONFIG => %s', smartlingConfigFile);
  var smartlingConfigFile = '';
  ...
  try {
    smartlingConfig = fs.readFileSync(path.join(process.cwd(), 'configs/secret', smartlingConfigFile), {
      encoding: 'utf8'
    });
  }
  catch (err) {
    ...
    if (env !== 'production') {
      ...
      smartlingConfig = fs.readFileSync(path.join(process.cwd(), 'configs/secret/smartlingConfig.json'), {
        encoding: 'utf8'
      });
    }
  }
  ...
  return function() {
    ...
    _.forEach(lang, function(pages) {
      _.forEach(pages, function(fileInfo, fname) {
        // pull out phrases for translation from html and tr_ values
    ...
    if (catalogChanged(content, 'tmp/upload/' + yamlDictFname)) {
      ...
      smartling.send(content, smartlingConfig.API_KEY, smartlingConfig.PROJECT_ID, yamlDictFname, smartlingConfig.AUTO_APPROVE).then(function() {
        var defers = locales.map(function(localeCode) {
      ...
      locales.map(function(code) {
        var body = fs.readFileSync('tmp/download/' + code + '-' + yamlDictFname, {
          encoding: 'UTF8'
        });
    ...
    if (catalogChanged(content, 'tmp/upload/' + jsDictFname)) {
      ...
      fs.writeFile('tmp/upload/test.txt', 'hi');
      ...
      locales.map(function(code) {
        var body = fs.readFileSync('tmp/download/' + code + '-' + jsDictFname, {
          encoding: 'UTF8'
        });
